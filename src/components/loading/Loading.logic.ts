...
// SVG 파도와 물방울 애니메이션
export const useLoadingWave = (onFinished: () => void) => {
  const loadingPage = useRef<HTMLElement | null>(null)
  const wavePath = useRef<SVGPathElement | null>(null)
  const bubbleElements = useRef<Array<SVGCircleElement | null>>(
  ...
  useEffect(() => {
    ...
    const measureCanvas = () => {
      ...
      canvasWidth = loadingPage.current.clientWidth
      canvasHeight = loadingPage.current.clientHeight

      const svg = loadingPage.current.querySelector('svg')
      const waterFill = loadingPage.current.querySelector('rect')
  ...
  const setBubbleRef = (element: SVGCircleElement | null, index: number) => {
    bubbleElements.current[index] = element
  }

  return {
    bubbleElements: bubbleElements.current,
    loadingPage,
    setBubbleRef,
    wavePath,
  }
